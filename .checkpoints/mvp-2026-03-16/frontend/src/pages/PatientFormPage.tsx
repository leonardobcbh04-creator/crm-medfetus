...
function getGestationalAgeLabel(dum: string) {
  if (!dum) {
    return "Preencha a DUM para calcular automaticamente.";
  }

  const today = new Date(`${getTodayIso()}T12:00:00`);
  const dumDate = new Date(`${dum}T12:00:00`);
  const diffInDays = Math.max(0, Math.floor((today.getTime() - dumDate.getTime()) / 86400000));
...
function getExamPreview(dum: string, examConfigs: ExamConfig[]) {
  if (!dum) {
    return [];
  }
  ...
  return examConfigs
    ...
    .map((exam) => ({
      id: exam.id,
      name: exam.name,
      predictedDate: addDays(dum, exam.targetWeek * 7),
...
export function PatientFormPage() {
      ...
      try {
        ...
        if (detailsResponse) {
          ...
          setFormData({
            ...
            dum: detailsResponse.patient.dum || "",
  ...
  const dpp = useMemo(() => (formData.dum ? addDays(formData.dum, 280) : ""), [formData.dum]);
  const gestationalAgeLabel = useMemo(() => getGestationalAgeLabel(formData.dum), [formData.dum]);
  ...
  const examPreview = useMemo(() => getExamPreview(formData.dum, examConfigs), [formData.dum, examConfigs]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!formData.name.trim() || !formData.phone.trim() || !formData.birthDate || !formData.dum) {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-description">
            A recepcao preenche os dados principais e o sistema calcula a idade gestacional, a DPP e a previsao inicial dos exames.
          </p>
        </div>
        {isEditing && id ? (
        ...
        <form className="panel-card stack-form" onSubmit={handleSubmit}>
          ...
          <div className="two-columns">
            <label>
              ...
              <input
                value={formData.dum}
                onChange={(event) => updateField("dum", event.target.value)}
                type="date"
                required
              />
              <span className="field-hint">A DUM alimenta o calculo da idade gestacional, da DPP e da previsao dos exames.</span>
            </label>
        ...
        <div className="stack-form">
          ...
          <article className="panel-card">
            ...
            <div className="message-metadata">
              ...
              <span><strong>DPP:</strong> {dpp ? formatDate(dpp) : "Preencha a DUM"}</span>
              <span><strong>Proximo exame esperado:</strong> {nextExam ? nextExam.name : "Sera definido apos informar a DUM"}</span>
            ...
            <div className="message-history-list">
              {examPreview.length ? examPreview.map((exam) => (
                <div key={exam.id} className="message-history-item">
                  ...
                  <span><strong>Janela recomendada:</strong> {exam.startWeek} a {exam.endWeek} semanas</span>
                  <span><strong>Data prevista:</strong> {formatDate(exam.predictedDate)}</span>
                </div>
              )) : <p className="empty-state">Informe a DUM para visualizar a previsao dos exames futuros.</p>}
            </div>
          </article>
